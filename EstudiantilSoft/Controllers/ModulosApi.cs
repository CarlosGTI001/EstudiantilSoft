using EstudiantilSoft.Data;
using EstudiantilSoft.Models;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Caching.Memory;
using NuGet.Packaging.Licenses;

namespace EstudiantilSoft.Controllers
{
    
    public class ModulosApi : Controller
    {
        private readonly EstudiantilSoftContext _context;

        public ModulosApi(EstudiantilSoftContext context)
        {
            _context = context;
        }

        [HttpPost]
        public IActionResult InsertarMaestros([FromBody] MaestroImagen maestros)
        {
            try
            {
                var replace = "";
                if (string.IsNullOrEmpty(maestros.maestroFoto))
                {
                    replace = "";
                }
                else
                {
                    replace = maestros.maestroFoto.Replace("data:image/png;base64,", "");
                    replace = replace.Replace("data:image/jpeg;base64,", "");
                    replace = replace.Replace("data:image/jpg;base64,", "");
                }
                _context.maestros.Add(new Maestros
                {
                    maestroNombre = maestros.maestroNombre,
                    maestroApellido = maestros.maestroApellido,
                    maestroCedula = maestros.maestroCedula,
                    numeroTelefono = maestros.numeroTelefono,
                    maestroDireccion = maestros.maestroDireccion,
                    maestroFoto = Convert.FromBase64String(replace),
                    asignaturaNombre = maestros.asignaturaNombre,
                    AsignaturaID = maestros.AsignaturaID,
                    FechaRegistro = DateTime.Now
                });
                _context.SaveChanges();
                return Json("{'Success':'true'}");
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
            }
        }

        [HttpPost]
        public IActionResult ModificarMaestros([FromBody] MaestroImagen maestros)
        {
            try
            {
                
                var _maestro = _context.maestros.Find(maestros.MaestroID);
                _maestro.maestroNombre = maestros.maestroNombre;
                _maestro.maestroApellido = maestros.maestroApellido;
                _maestro.maestroCedula = maestros.maestroCedula;
                _maestro.numeroTelefono = maestros.numeroTelefono;
                _maestro.maestroDireccion = maestros.maestroDireccion;
                if(!string.IsNullOrEmpty(maestros.maestroFoto))
                {
                    var replace = maestros.maestroFoto.Replace("data:image/png;base64,", "");
                    replace = replace.Replace("data:image/jpeg;base64,", "");
                    replace = replace.Replace("data:image/jpg;base64,", "");
                    _maestro.maestroFoto = Convert.FromBase64String(replace);
                }
                else
                {
                    
                }
                _maestro.asignaturaNombre = maestros.asignaturaNombre;
                _maestro.AsignaturaID = maestros.AsignaturaID;
                _context.maestros.Update(_maestro);
                _context.SaveChanges();
                return Json("{'Success':'true'}");
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
            }
        }

        [HttpPost]
        public IActionResult InsertarEstudiantes([FromBody] EstudianteImagen estudiantes)
        {
            try
            {
                var replace = "";
                if (string.IsNullOrEmpty(estudiantes.estudianteFoto))
                {
                    replace = "";
                }
                else
                {
                    replace = estudiantes.estudianteFoto.Replace("data:image/png;base64,", "");
                    replace = replace.Replace("data:image/jpeg;base64,", "");
                    replace = replace.Replace("data:image/jpg;base64,", "");
                }
                _context.Estudiantes.Add(new Estudiantes
                {
                    estudianteNombre = estudiantes.estudianteNombre,
                    estudianteApellido = estudiantes.estudianteApellido,
                    numeroTelefono = estudiantes.numeroTelefono,
                    estudianteDireccion = estudiantes.estudianteDireccion,
                    estudianteFoto = Convert.FromBase64String(replace),
                    seccionNombre = estudiantes.seccionNombre,
                    SeccionID = estudiantes.SeccionID,
                    FechaRegistro = DateTime.Now
                });

                _context.SaveChanges();
                return Json("{'Success':'true'}");
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
            }
        }

        [HttpPost]
        public IActionResult ModificarEstudiantes([FromBody] EstudianteImagen estudiantes)
        {
            try
            {

                var _estudiante = _context.Estudiantes.Find(estudiantes.EstudianteID);
                _estudiante.estudianteNombre = estudiantes.estudianteNombre;
                _estudiante.estudianteApellido = estudiantes.estudianteApellido;
                _estudiante.numeroTelefono = estudiantes.numeroTelefono;
                _estudiante.estudianteDireccion = estudiantes.estudianteDireccion;
                if (!string.IsNullOrEmpty(estudiantes.estudianteFoto))
                {
                    var replace = estudiantes.estudianteFoto.Replace("data:image/png;base64,", "");
                    replace = replace.Replace("data:image/jpeg;base64,", "");
                    replace = replace.Replace("data:image/jpg;base64,", "");
                    _estudiante.estudianteFoto = Convert.FromBase64String(replace);
                }
                else
                {

                }
                _estudiante.seccionNombre = estudiantes.seccionNombre;
                _estudiante.SeccionID = estudiantes.SeccionID;
                _context.Estudiantes.Update(_estudiante);
                _context.SaveChanges();
                return Json("{'Success':'true'}");
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
            }
        }

        public IActionResult CalcularDeudas(int EstudianteID)
        {
            var deuda = _context.pagos.Where(a => a.EstudianteID == EstudianteID).Select(a => a.DeudaValor).Sum();
            var d2 = new DeudaN();
            d2.Deuda = deuda;
            return Json(d2);
        }

        public IActionResult LeerDeudasEstudiante(int EstudianteID)
        {
            var deuda = _context.pagos.Where(a => a.EstudianteID == EstudianteID);
            return Json(deuda);
        }

        public IActionResult LeerDeuda(int DeudaID)
        {
            var deuda = _context.pagos.Where(a => a.DeudaID == DeudaID);
            return Json(deuda);
        }

        [HttpPost]
        public IActionResult PagarDeuda([FromBody] deudaN deuda)
        {
            try
            {
                var pagos = _context.pagos.Select(a => a).Where(a => a.DeudaID == deuda.DeudaID).FirstOrDefault();
                pagos.DeudaValor = deuda.DeudaValor;
                _context.pagos.Update(pagos);
                _context.SaveChanges();
                return Json("{'listo':true}");
            }
            catch
            {
                return BadRequest();
            }
        }
    }
    public class deudaN
    {
        public int DeudaID { get; set; }
        public decimal DeudaValor { get; set; }
    }

    public class DeudaN
    {
        public decimal Deuda { get; set; }
    }
}

